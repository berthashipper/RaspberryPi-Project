...
def display_bbc_news():
    ...
    def extract_keyword_phrase(title, description):
        ...
        keyword = proper_chunks[0] if proper_chunks else (next((chunk.text for chunk in doc.noun_chunks if 1 <= len(chunk.text.split()) <= 5), None))
        if keyword:
            keyword_words = [w for w in keyword.split() if w.lower() not in months and w.lower() not in stop_words and w.lower() != "news" and not re.fullmatch(r"[\d\W]+", w)]
            keyword = " ".join(keyword_words).strip() or None
        if not keyword:
            for word in title.split():
                w = word.lower().strip(string.punctuation)
                if w not in months and w not in stop_words and w != "news" and len(w) > 1:
                    keyword = word
                    break
    ...
    def sentiment_to_color(article):
        ...
        if any(t in text for t in war_terms):
            return [255, 0, 0], "war/disaster"
        elif any(t in text for t in positive_terms):
            return [0, 255, 0], "positive"
        else:
            score = article['textblob_score']
            if score > 0.2: return [0, 255, 0], f"textblob positive ({score:.2f})"
            elif score < -0.2: return [255, 0, 0], f"textblob negative ({score:.2f})"
            else: return [255, 255, 0], f"textblob neutral ({score:.2f})"
    ...
    # --- display intro ---
    if articles:
        dates = [datetime.fromisoformat(a['published_at'].replace('Z','+00:00')) for a in articles]
        intro_message = f"BBC Articles {min(dates).strftime('%b %d')} to {max(dates).strftime('%b %d')}"
        sense.clear([0,0,255])
        sense.show_message(intro_message, scroll_speed=0.01, text_colour=[255,255,255])
        sense.clear()
        # white center dot
    ...
    # --- display keywords before the grid ---
    for article in articles:
        keyword = article['keyword']
        color, _ = sentiment_to_color(article)
        sense.clear(color)
        sense.show_message(keyword, scroll_speed=0.01, text_colour=color)
        time.sleep(0.3)
        for i in range(CENTER_START, CENTER_START + 4):
            for j in range(CENTER_START, CENTER_START + 4):
                sense.set_pixel(i,j,[255,255,255])
    ...
    while True:
        for event in sense.stick.get_events():
            ...
            if event.direction == 'up' and selected >= 3:
                ...
                # call function to display full article
